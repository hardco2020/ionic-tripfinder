  ...
  getAttraction(Aid): Promise<Attraction> {
    return this.storage.executeSql('SELECT * FROM AttractionInfo WHERE Aid = ?', [Aid]).then(res => { 
      return {
        ...
        distance: res.rows.item(0).distance,
      ...
      if (res.rows.length > 0) {
        for (var i = 0; i < res.rows.length; i++) { 
          items.push({  //getting all the columns by column names
            ...
            favorite: res.rows.item(i).favorite,  
            distance: res.rows.item(i).distance,
            sweet: res.rows.item(i).sweet,
            salty: res.rows.item(i).salty,
            ...
            netbeauty: res.rows.item(i).netbeauty,
            vendor: res.rows.item(i).vendor,
            restaurant: res.rows.item(i).restaurant,
  ...
  getRestaurant(Aid): Promise<Restaurant> {
    return this.storage.executeSql('SELECT * FROM FoodInfo WHERE Aid = ?', [Aid]).then(res => { 
      return {
        ...
        favorite: res.rows.item(0).favorite,  
        distance: res.rows.item(0).distance,
        sweet: res.rows.item(0).sweet,
        salty: res.rows.item(0).salty,
        cheap: res.rows.item(0).cheap,
        expensive: res.rows.item(0).expensive,
        buffet: res.rows.item(0).buffet,  
        chinese: res.rows.item(0).chinese,
        western: res.rows.item(0).western,
        japanKorean: res.rows.item(0).japanKorean,
        southeastAsian: res.rows.item(0).southeastAsian,
        netbeauty: res.rows.item(0).netbeauty,
        vendor: res.rows.item(0).vendor,
        restaurant: res.rows.item(0).restaurant,
        alcohol: res.rows.item(0).alcohol,
